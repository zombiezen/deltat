...
func TestParseTime(t *testing.T) {
	...
	for _, test := range tests {
		got, err := parseTime(test.now, test.s, false)
		if !got.Equal(test.want) || (err != nil) != test.err {
			if test.err {
				t.Errorf("parseTime(%v, %q, false) = %v, %v; want _, <error>",
					test.now, test.s, got, err)
			} else {
				t.Errorf("parseTime(%v, %q, false) = %v, %v; want %v, <nil>",
					test.now, test.s, got, err, test.want)
			}
		}

		got, err = parseTime(test.now, test.s, true)
		if test.hasDate && !got.Equal(test.want) ||
			!test.hasDate && !got.IsZero() ||
			(err != nil) != (test.err || !test.hasDate) {
			if test.err {
				t.Errorf("parseTime(%v, %q, true) = %v, %v; want _, <error>",
					test.now, test.s, got, err)
			} else {
				t.Errorf("parseTime(%v, %q, true) = %v, %v; want %v, <nil>",
					test.now, test.s, got, err, test.want)
			}
		}
	}
